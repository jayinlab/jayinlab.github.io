...
- CL: **간접** — OpenCL API 호출 패턴(clEnqueueNDRangeKernel 빈도) 관점
- VK: **직접** — command buffer/queue submit 단위가 오버헤드에 직접 영향
- ANGLE: **간접** — OpenCL 호출을 Vulkan submit 경로로 매핑하는 계층
- SPV: **참고** — 셰이더 IR 자체보다 제출 빈도가 이번 병목의 핵심
- PM4: **간접** — 최종 command stream 분할 증가로 실행 연속성 저하 가능
- PERF: **직접** — tiny dispatch 성능 저하의 중심 축
